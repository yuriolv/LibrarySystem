...
public class Rents {  
    ...
    public boolean create(RentBook rent, DataBase db){
        ...
        try {
            ResultSet rs = db.requestSQL("SELECT id_livro FROM livro WHERE titulo = " + rent.getTitulo());
            rs.next();
    ...
    public ArrayList<RentBook> read(DataBase db, Optional<ArrayList<String>> conditions){
        ...
        try {
            ...
            while (rs.next()) {
                ...
                RentBook rent = new RentBook(matricula, titulo_livro, tipo_usuario, data_aluguel, data_devolucao);
                rents.add(rent);
            }
